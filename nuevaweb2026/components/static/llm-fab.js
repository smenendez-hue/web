/* ============================================================
   ...
   Uso: <llm-fab base="../../"></llm-fab>
   Carga yiqi-design.md + execution.md y los copia al clipboard.
   DS v1.2.5
   ============================================================ */
    ...
    btn.addEventListener('click', async () => {
      try {
        const [design, execution] = await Promise.all([
          fetch(base + 'content/design-system/yiqi-design.md').then(r => { if (!r.ok) throw new Error(r.status); return r.text(); }),
          fetch(base + 'content/prompts/execution.md').then(r => { if (!r.ok) throw new Error(r.status); return r.text(); }),
        ]);
        const full = design + '\n\n---\n\n' + execution;
