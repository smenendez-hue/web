...
(function () {
  ...
  class AppCard extends HTMLElement {
    connectedCallback() {
      const a = (window.YIQI_APP_BY_ID || {})[this.getAttribute('app')];
      this.innerHTML = a ? cardHTML(a) : '';
    }
  }
  if (!customElements.get('app-card')) customElements.define('app-card', AppCard);

  // Para usos sin componente (ej. armar el carrusel por JS)
  window.YIQI_APP_CARD_HTML = cardHTML;

  if (!window.__appCardNav) {
    window.__appCardNav = true;
    document.addEventListener('click', e => { const c = e.target.closest('.app-card[data-href]'); if (c) location.href = c.dataset.href; });
    document.addEventListener('keydown', e => { if (e.key === 'Enter') { const c = e.target.closest('.app-card[data-href]'); if (c) location.href = c.dataset.href; } });
  }
})();
